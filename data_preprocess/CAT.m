...
nor_rows = round(max_pd/width_row);


proj_blender = cell(samples_cycle, 1);
consider_pitch = 0;
%%%拼接实现，在矩阵水平方向拼接（也即投影的rows方向）%%%
%%原来是按z降序，投影水平朝右拼接；现在变成z轴升序，需要朝左拼接。
for i = 1:length(file)
    ...
    if i<=samples_cycle
        ...
        if consider_pitch
            ...
            % 这里是水平拼接，第二维在变长..
            %%由于对z重排了，这里拼接时也要翻一下
            proj_blender{stem}.pixel = [proj_all(:, 1:effect_rows, i),proj_blender{stem}.pixel];
        else
            proj_blender{stem}.pixel = [proj_all(:,:,i),proj_blender{stem}.pixel];
        end
    end

end
close all;
imshow(proj_blender{500}.pixel,[],'Colormap',gray,'InitialMagnification','fit');
colorbar;
imshow(img,[],'Colormap',gray,'InitialMagnification','fit');
colorbar;
  


%%%将拼接好后的投影根据其z轴位置决定它在全平面投影内的位置,注意z是逐渐变小的
%%投影缺失的地方统一用0替代
% plane_zmax = max(zsorted);
% plane_zmin = min(zsorted);
% proj_full_plane = zeros(col,nor_rows,samples_cycle);
% for i = 1:samples_cycle
%     gap = plane_zmax - proj_blender{i}.zmax;
%     gap_pixel = round(gap/width_row);
%     if gap_pixel == 0
%         gap_pixel = 1;
%     end
%     span = size(proj_blender{i}.pixel,2);
%     proj_full_plane(:,gap_pixel:gap_pixel+span-1,i) = proj_blender{i}.pixel;
% end
...
% start = round((plane_zmax-cut(2)/width_row));
% fin_span = nor_size(2);
% for j=1:samples_cycle
%     proj_full_prune(j) = proj_full_plane(:,start:start+fin_span-1,i);
...
json_data.scanner.mode = "cone";  % or "cone" depending on your scanner
